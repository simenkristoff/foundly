...
/**
 * The Class DepthManager. Creates a shadow effect for a given node and a specified depth level.
 * depth levels are {0,1,2,3,4,5}
 *
 */
public class DepthManager {

  private static DropShadow[] depth =
      new DropShadow[] {new DropShadow(BlurType.GAUSSIAN, Color.rgb(0, 0, 0, 0), 0, 0, 0, 0),
  ...
  /**
   * Adds shadow effect to the node, however the shadow is not real ( gets affected with node
   * transformations).
   * <p>
   * use {@link #createMaterialNode(Node, int)} instead to generate a real shadow
   * </p>
   * @param control node to add shadow
   * @param level depth level in which to add shadow
   */
  public static void setDepth(Node control, int level) {
    level = level < 0 ? 0 : level;
    level = level > 5 ? 5 : level;
    control.setEffect(
        new DropShadow(BlurType.GAUSSIAN, depth[level].getColor(), depth[level].getRadius(),
            depth[level].getSpread(), depth[level].getOffsetX(), depth[level].getOffsetY()));
  }
  ...
  /**
   * Generates a new container node in order to prevent control transformation to be applied to the
   * shadow effect (which makes it looks as a real shadow).
   * 
   * @param control node to add shadow
   * @param level depth level in which to add shadow
   * 
   * @return new container node with shadow effect
   */
